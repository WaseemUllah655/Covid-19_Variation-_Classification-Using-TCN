...
def custom_loss1(y_true, y_pred):
    ...
    loss = pow((y_pred[0] - y_true), 2) / (2 * pow(y_pred[1], 2)) + tf.math.log(y_pred[1])
    return loss
...
#     cross_ent = bce(y_true, y_pred)

#     _, _, count = tf.keras.unique_with_counts(tf.keras.initializers.Constant(y_pred))
#     # [1 2 2 1]
#     prob = count / tf.keras.reduce_sum(count)
#     # [0.16666667 0.33333333 0.33333333 0.16666667]
#     entro = -tf.keras.reduce_sum(prob * tf.log(prob))
#     loss = cross_ent + entro
#     return loss

model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy'])

# model.compile(loss= custom_loss1, optimizer=opt, metrics=['accuracy'])
...
fileName = './Classification_nucleotide_Model_1'
