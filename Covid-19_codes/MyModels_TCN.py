...
class LSTMModel(tf.keras.Model):
  ...
  def __init__(self):
    ...
    self.LSTM1 = TCN(nb_filters = 256, nb_stacks = 1, return_sequences = True,
          use_skip_connections = True, activation = "relu", dropout_rate = 0.2)
    self.LSTM2 = TCN(nb_filters = 256, nb_stacks = 1, return_sequences = False,
          use_skip_connections = True, activation = "relu", dropout_rate = 0.2)
    self.dense1 = tf.keras.layers.Dense(7, activation=tf.nn.softmax)
  ...
  # def call(self, inputs, training=False):
  #   x = tf.tensorflow.keras.layers.Input(shape=(X_train.shape[1], X_train.shape[2]))
  #   x = self.LSTM1(x)
  #   x = self.LSTM2(x)
  #   x = tf.keras.layers.Attention()([x,x])
  #   x = tf.keras.layers.Flatten()(x)
